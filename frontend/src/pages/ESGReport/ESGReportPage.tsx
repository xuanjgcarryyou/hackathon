...
export default function ESGReportPage() {
  ...
  async function handleExport(reportId: string) {
    ...
    try {
      ...
      triggerDownload(new Blob([data], { type: 'application/json' }), `esg-report-${reportId.slice(0, 8)}.json`)
    } catch { /* silent */ } finally { setExporting(false) }
  }

  async function handleExportXlsx(reportId: string) {
    setExportingXlsx(true)
    try {
      const { data } = await api.exportESGReportXlsx(reportId)
      triggerDownload(new Blob([data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }), `esg-report-${reportId.slice(0, 8)}.xlsx`)
    } catch { /* silent */ } finally { setExportingXlsx(false) }
  }
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                {report.reportId && (
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleExportXlsx(report.reportId)}
                      disabled={exportingXlsx}
                      className="flex items-center gap-1.5 text-xs font-medium text-emerald-700 border border-emerald-400 bg-emerald-50 hover:bg-emerald-100 disabled:opacity-50 px-3 py-1.5 rounded-lg transition-colors cursor-pointer"
                    >
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                        <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/>
                      </svg>
                      {exportingXlsx ? '匯出中...' : '下載 Excel'}
                    </button>
                    <button
